...
export async function GET(context: APIContext) {
  ...
  // SFW作品のみフィルタリング
  const sfwPosts = allPosts.filter(post => {
    const tags = post.frontmatter.tags || [];
    return !tags.includes("R18") && !tags.includes("R18G");
  });

  // 日付順にソート（新しい順）
  const sortedPosts = sfwPosts.sort((a, b) => {
    return new Date(b.frontmatter.date).getTime() - new Date(a.frontmatter.date).getTime();
  });

  return rss({
    title: 'Dragontail - しもふみのイラストサイト',
    description: 'ドラゴンガールとケモミミが好きな絵描き、しもふみさんによるイラストサイト。オリジナルも二次創作も。全年齢向け作品。',
