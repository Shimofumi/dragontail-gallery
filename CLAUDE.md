...
### ステップ 3 — MD テンプレートを書き出す

```bash
node scripts/new-artwork.js --write
```

### ステップ 4 — ⚠️ 必ずユーザーが手動で MD を確認・編集する（ここで止まること）

**`--write` 後に自動で ship まで進めてはいけない。** 必ずユーザーに編集を促し、「確認できたら ship して」と伝えること。

生成 MD の確認ポイントをユーザーに伝える：

| フィールド | 自動生成内容 | 確認・修正が必要な理由 |
|-----------|------------|----------------------|
| `title` | ファイル名からキャラ名を推定 | ファイル名と作品タイトルは一致しないことが多い |
| `tags` | 既存 MD のキャラ名部分一致で候補提示 | 精度は低い。`_study` 系ファイルはキャラ固定でなくシチュエーション次第 |
| `date` | 実行日 | 新着順・RSS の基準。古い作品を今日付けで登録したい場合は要調整 |
| `additionalImages` | ファイル名パターンから推定 | 実際のファイル名と合っているか目視確認 |

特に注意するケース：
- `_study` `_practice` 系 → タグはキャラではなく「かんたん絵」等になる可能性
- 複数キャラが登場する作品 → タグ候補が片方のキャラしか拾えていないことがある
- `additionalImages` がないはずなのに入っている、またはその逆

📎 追加画像（📎 表示のグループ）は自動で MD 生成されない。手動で既存 MD を開いて `additionalImages` に追記すること。

### ステップ 5 — ユーザー確認後にビルド & デプロイ

ユーザーから「確認した」「ship して」などの指示を受けてから実行する：
